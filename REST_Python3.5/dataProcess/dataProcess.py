...
def dataDecodeFromNet(contractKLineLst):
    ...
    if len(maxLine) or len(lowLine):
        lastKline = g_KlineUpDwnSequence & 0x1
        ...
        elif nextDoing in [myEnum.RISING_ONLY_DWN, myEnum.RISING_EQ_DWN, myEnum.RISING_EQ_EQ, myEnum.RISING_EQ_UP,
                           myEnum.RISING_HAMMER_UP, myEnum.RISING_HAMMER_DWN, myEnum.RISING_ONLY_DWN]:
        ...
        g_LastKline = copy.deepcopy(contractKLineLst[index])
        ...
        for index in range(len(contractKLineLst) - 1):
            currentKline = getUpOrDwn(contractKLineLst[index])
            g_KlineUpDwnSequence = g_KlineUpDwnSequence << 1 | currentKline
            tempKline = copy.deepcopy(contractKLineLst[index])
            if lastKline > currentKline:  # 阳阴
...
def testdataDecodeFromNet():
    ...
    g_PrintFlag = False
    inputdata = [ \
        {'amount': 17137.364187577074, 'close': 314.737, 'count': 3028, 'high': 317.259, 'id': 1569121200,
         'low': 314.126, 'open': 317.144, 'vol': 540888}, \
        {'amount': 17796.991253268166, 'close': 315.697, 'count': 2957, 'high': 316.081, 'id': 1569124800,
         'low': 313.673, 'open': 314.738, 'vol': 560246}, \
        {'amount': 13152.020335745643, 'close': 317.092, 'count': 2361, 'high': 318.04, 'id': 1569128400,
         'low': 315.174, 'open': 315.71, 'vol': 416880}, \
        {'amount': 8614.527755853285, 'close': 315.642, 'count': 1422, 'high': 317.356, 'id': 1569132000,
         'low': 315.091, 'open': 317.031, 'vol': 272368}, \
        {'amount': 12063.814470932364, 'close': 317.906, 'count': 1993, 'high': 318.482, 'id': 1569135600,
         'low': 315.461, 'open': 315.508, 'vol': 383192}]
    g_KlineUpDwnSequence = g_KlineUpDwnSequence
    dataDecodeFromNet(inputdata)

    testCase("g_KlineUpDwnSquence by dataDecodeFromNet", g_KlineUpDwnSequence, 0x6)
    ...
    inputdata = [ \
        ...
        {'amount': 12063.814470932364, 'close': 317.906, 'count': 1993, 'high': 318.482, 'id': 1569135600,
         'low': 315.461, 'open': 315.508, 'vol': 383192}, \
        {'amount': 12063.814470932364, 'close': 317.906, 'count': 1993, 'high': 318.482, 'id': 1569135600,
         'low': 315.461, 'open': 317.906, 'vol': 383192}]
    dataDecodeFromNet(inputdata)
    testCase("g_KlineUpDwnSquence by dataDecodeFromNet", g_KlineUpDwnSequence, 0xD)
